...
自研 ROS 2 代码全部放本目录（一个功能一个包）。方案依据 SSOT（`../总体方案与执行计划.md`）；
...
- ~~M2 真感知节点~~ ✅ **2026-09-13 代码落地**：`bucket_perception_node`（双通道
  ...
  **待上机过契约校验接状态机**（模型/标定/外场项见《01_视觉感知/视觉算法设计.md》§8/§9）
...
mkdir -p ~/ros2_ws/src
cp -r /path/to/03_机载软件/cuadc_mission ~/ros2_ws/src/
...
# 3) 联跑顺序：
#    ① Gazebo + ArduPilot SITL（见 08_参考资料/外部仓库/hgd_cudac/sim/cuadc_sim 的说明）
