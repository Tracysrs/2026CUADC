...
CUADC"多旋翼侦察与救援"赛项的仿真场地。从 `08_参考资料/外部仓库/hgd_cudac/sim/cuadc_sim` 原样
...
与项目选型定案（新版 Gazebo + ArduPilot SITL + MAVROS，见 总体方案与执行计划.md §7）同一体系。
...
- **重新随机布设场地**：
  ...
  ```bash
  cd 04_仿真/仿真环境/cuadc_rescue_sim
...
模型与贴图来自 `08_参考资料/外部仓库/hgd_cudac`（上游基于 ardupilot_gazebo 的 Iris 派生，
