...
def main():
    ...
    for name, want in WANT.items():
        ...
        if int(cur) == want:
            ...
            changed = changed or (got is not None and int(got) != want)
            changed = changed or got is None

    if changed:
        print("\n参数有变动，重启飞控生效...")
        time.sleep(1)
        m.mav.command_long_send(m.target_system, m.target_component,
                                mavutil.mavlink.MAV_CMD_PREFLIGHT_REBOOT,
                                0, 1, 0, 0, 0, 0, 0, 0)
        m.close()
        print("等 25s 待飞控重启...")
        time.sleep(25)
    ...
    for mid, iv in [(24, 1000000), (116, 1000000),
    ...
    if g2:
        ...
        print("❌ rover 仍无数据 → 按序排查：")
        print("   1. rover 三颗 LED 是否亮（电源/LORA/RTK）")
        print("   2. 6P GH 线针序（规格书存疑项：万用表实测 5V/GND，TX/RX 交叉）")
