...
> 背景与版本锚点见 `01_设计/SIH台架仿真计划.md`。SIH = 仿真物理模型跑在真 V6X 内，
...
- 产物 `~/sih_firmware_out/arducopter_SIH_CUAV-V6X-v2.apj` + SHA256 回拷归档到
  `01_设计/firmware/` 后再进 §2 刷入。

## 1. 备份（刷机前，一次）

```bash
python ops.py            # 选 params-export —— 全量备份到 01_设计/参数备份/
...
1. Mission Planner → 设置 → 安装固件 → 加载自定义固件 →
   选 `01_设计/firmware/arducopter_beta_CUAV-V6X-v2_SIH_台架专用.apj`；
2. 重插 USB 重连；
3. 导入 `01_设计/V6X_ardupilot_params.param` → 写参数 → 重启 →
   `python verify_params.py --diff 01_设计/V6X_ardupilot_params.param` 全项一致；
4. 导入 `01_设计/V6X_SIH_bench.param` → 写参数 → 重启；
...
1. 重刷 `01_设计/firmware/arducopter_beta_CUAV-V6X-v2.apj`（锁定 beta）；
