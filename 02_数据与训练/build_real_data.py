...
⚠️ 2026-09-12 类别表勘正（见 01_视觉模块 §2.1 / SSOT §4.4）：
train/ 里现存的标签 txt 是按【旧 GHS 表】打的类别号（旧 2=腐蚀品 3=刺激性
4=感染性物品 5=氧化剂），与本脚本现在的 CLASS_NAMES（附件11 实表，
2=刺激性 3=放射性物品 4=腐蚀品 5=生物危害）**id 2~5 含义错位**。
直接重跑本脚本 = 错位标签配上正确名字，更隐蔽更危险。重跑前必须先把
train/labels 按图案重映射（参照另一工作区 hazard_hgd_10cls_v2/mapped_dataset
的做法：按图案重排 + 定向修错标 + labels_backup 备份）。
本脚本当前的可用价值 = 切分逻辑复现（防连续帧泄题的三条纪律）。
...
# barrel 沿用 v1 11 类检测头习惯保留在 10 号位
CLASS_NAMES = {
    0: "爆炸品", 1: "不燃气体", 2: "刺激性", 3: "放射性物品", 4: "腐蚀品",
    5: "生物危害", 6: "遇湿易燃物品", 7: "有毒品", 8: "自燃物品", 9: "易燃",
    10: "barrel",
}
